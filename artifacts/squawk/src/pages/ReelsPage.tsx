    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
          {!isReal ? (
            <div className="text-center py-12 text-white/40 text-sm">Demo post — no comments available</div>
          ) : isLoading ? (
            <div className="flex justify-center py-12">
              <div className="w-6 h-6 border-2 border-primary border-t-transparent rounded-full animate-spin" />
            </div>
          ) : comments.length === 0 ? (
            <div className="text-center py-12 text-white/40 text-sm">No comments yet. Be the first!</div>
          ) : (
            comments.map((c: any) => (
...
function Reel({ post, muted, setMuted }: { post: any; muted: boolean; setMuted: (m: boolean) => void }) {
  ...
  return (
    <div ref={containerRef} className="h-[100dvh] w-full snap-start relative flex items-center justify-center bg-black">
      {/* 9:16 constrained frame — full width on mobile, centered pillarbox on desktop */}
