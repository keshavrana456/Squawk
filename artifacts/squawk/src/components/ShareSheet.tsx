...
import {
  FaXTwitter, FaWhatsapp, FaTelegram, FaFacebook,
  FaReddit, FaDiscord, FaInstagram, FaSnapchat,
  FaLinkedin, FaPinterest, FaThreads, FaTiktok,
...
const PLATFORMS: Platform[] = [
  { name: "X", Icon: FaXTwitter, iconColor: "#ffffff", bgColor: "#000000",
    onClick: (url, text) => openUrl(`https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`) },
  { name: "WhatsApp", Icon: FaWhatsapp, iconColor: "#ffffff", bgColor: "#25D366",
    onClick: (url, text) => openUrl(`https://wa.me/?text=${encodeURIComponent(text + " " + url)}`) },
  { name: "Telegram", Icon: FaTelegram, iconColor: "#ffffff", bgColor: "#2AABEE",
    onClick: (url, text) => openUrl(`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`) },
  { name: "Discord", Icon: FaDiscord, iconColor: "#ffffff", bgColor: "#5865F2",
    onClick: () => openUrl("https://discord.com/channels/@me") },
  { name: "Instagram", Icon: FaInstagram, iconColor: "#ffffff",
    bgColor: "linear-gradient(135deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%)",
    onClick: async (url) => { try { await navigator.clipboard.writeText(url); alert("Link copied! Open Instagram and paste it."); } catch {} } },
  { name: "Snapchat", Icon: FaSnapchat, iconColor: "#000000", bgColor: "#FFFC00",
    onClick: (url) => openUrl(`https://www.snapchat.com/scan?attachmentUrl=${encodeURIComponent(url)}`) },
  { name: "Facebook", Icon: FaFacebook, iconColor: "#ffffff", bgColor: "#1877F2",
    onClick: (url) => openUrl(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`) },
  { name: "Reddit", Icon: FaReddit, iconColor: "#ffffff", bgColor: "#FF4500",
    onClick: (url, text) => openUrl(`https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(text)}`) },
  { name: "Threads", Icon: FaThreads, iconColor: "#ffffff", bgColor: "#000000",
    onClick: (url, text) => openUrl(`https://www.threads.net/intent/post?text=${encodeURIComponent(text + " " + url)}`) },
  { name: "LinkedIn", Icon: FaLinkedin, iconColor: "#ffffff", bgColor: "#0A66C2",
    onClick: (url) => openUrl(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`) },
  { name: "Pinterest", Icon: FaPinterest, iconColor: "#ffffff", bgColor: "#E60023",
    onClick: (url, text) => openUrl(`https://pinterest.com/pin/create/button/?url=${encodeURIComponent(url)}&description=${encodeURIComponent(text)}`) },
  { name: "TikTok", Icon: FaTiktok, iconColor: "#ffffff", bgColor: "#010101",
    onClick: async (url) => { try { await navigator.clipboard.writeText(url); alert("Link copied! Open TikTok and paste it."); } catch {} } },
];
        ...
        <div className="px-4 pb-6 space-y-4">
                  ...
                  return (
                    ...
                    >
                      <div className="relative">
                        <Avatar className={`w-14 h-14 border-2 transition-all ${alreadySent ? "border-green-500" : "border-border group-hover:border-primary"}`}>
                      ...
                      {alreadySent && <span className="text-xs text-green-500 font-medium">Sent</span>}
                    </button>
                  );
                })}
              </div>
            ...
            {PLATFORMS.map((platform) => (
              ...
              >
                <div
                  ...
                  style={platform.bgColor.startsWith("linear") ? { background: platform.bgColor } : { backgroundColor: platform.bgColor }}
                >
                  <platform.Icon size={26} color={platform.iconColor} />
                </div>
                <span className="text-xs text-muted-foreground leading-tight text-center">{platform.name}</span>
              </button>
            ))}
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-background flex items-center justify-center flex-shrink-0 border border-border">
              {copied ? <Check size={18} className="text-green-500" /> : <Copy size={18} className="text-foreground" />}
            </div>
            <div className="flex flex-col items-start min-w-0">
              <span className="text-sm font-semibold">Copy link</span>
              <span className="text-xs text-muted-foreground truncate w-full">{postUrl}</span>
            </div>
            {copied && <span className="ml-auto text-xs text-green-500 font-medium flex-shrink-0">Copied!</span>}
          </button>
        </div>
      </DrawerContent>
    </Drawer>
