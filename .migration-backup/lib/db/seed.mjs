...
async function seed() {
  ...
  try {
    ...
    // Seed bots follow all existing real (non-seed) users and vice versa
    console.log("Wiring bot follows for existing real users...");
    const { rows: realUsers } = await client.query(
      `SELECT id FROM users WHERE clerk_id NOT LIKE 'seed_%'`
    );
    for (const realUser of realUsers) {
      const bots = pickRandomArr(userIds, 10);
